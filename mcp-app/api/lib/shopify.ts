...
const CATEGORY_PATTERNS: Array<
	Omit<CatalogCategory, "titlePattern"> & { titlePattern: RegExp }
> = [
	{
		id: "calçado",
		label: "Calçados",
		terms: ["shoes", "sneakers", "canvas shoes", "flip flops", "slides"],
		titlePattern: /canvas shoe|sneaker|flip flop|slide|high top/i,
	},
	{
		id: "camiseta",
		label: "Camisetas",
		terms: ["t-shirt", "tee", "shirt"],
		titlePattern: /t-shirt|tee|shirt/i,
	},
	{
		id: "moletom",
		label: "Moletons e suéteres",
		terms: ["hoodie", "sweatshirt"],
		titlePattern: /hoodie|sweatshirt/i,
	},
	{
		id: "jaqueta",
		label: "Jaquetas",
		terms: ["jacket"],
		titlePattern: /jacket/i,
	},
	{
		id: "calça",
		label: "Calças",
		terms: ["pants", "sweatpants", "joggers"],
		titlePattern: /sweatpant|jogger|pant/i,
	},
	{
		id: "caneca",
		label: "Canecas e garrafas",
		terms: ["mug", "tumbler", "bottle", "water bottle"],
		titlePattern: /mug|tumbler|bottle/i,
	},
	{
		id: "bolsa",
		label: "Bolsas e mochilas",
		terms: ["backpack", "tote bag", "bag"],
		titlePattern: /backpack|tote|bag/i,
	},
	{
		id: "boné",
		label: "Bonés e chapéus",
		terms: ["hat", "bucket hat", "winter hat", "cap"],
		titlePattern: /hat|cap/i,
	},
	{
		id: "caderno",
		label: "Cadernos",
		terms: ["notebook"],
		titlePattern: /notebook/i,
	},
	{ id: "caneta", label: "Canetas", terms: ["pen"], titlePattern: /pen/i },
	{
		...
		label: "Almofadas",
		terms: ["pillow"],
		titlePattern: /pillow/i,
	},
	{
		id: "capa",
		label: "Capas de celular",
		terms: ["snap case", "case"],
		titlePattern: /case/i,
	},
	{
		id: "adesivo",
	...
	for (const product of catalog) {
		...
		for (const term of terms) {
			...
			if (isMulti) {
				...
				titleTokens.some(
					(tk) =>
						tk.length >= 4 && (tk.startsWith(term) || term.startsWith(tk)),
				)
			) {
				hits++;
				termHit = true;
		...
		const score =
			Math.round((hits / terms.length + exactHits / terms.length) * 50) / 100;
		results.push({
			product,
			score,
