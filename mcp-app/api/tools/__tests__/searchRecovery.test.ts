...
import { restoreFetchAfterEach, stubNetwork, TEST_CATALOG } from "./helpers.ts";
...
describe("search_recovery tool", () => {
	...
	describe("happy path (LLM available)", () => {
		...
		it("returns 3+ grounded products with a session id", async () => {
			...
			expect(res.products[0]).toHaveProperty("description");
	...
	describe("zero-results recovery", () => {
		...
		it("onboards greetings with grounded products and a helpful question", async () => {
			const res = await run({ query: "hi" });
			expect(res.products.length).toBeGreaterThanOrEqual(1);
			expect(res.explanation).toMatch(/oi|opções/i);
			expect(res.follow_up_question).toMatch(
				/como posso ajudar|produto|preço/i,
			);
	...
	describe("input validation", () => {
		it("rejects empty/whitespace queries via schema", () => {
			...
			const { searchRecoveryInputSchema } =
				require("../searchRecovery.ts") as typeof import("../searchRecovery.ts");
