...
describe("converse tool", () => {
  ...
  describe("validation & errors", () => {
    ...
    it("throws for an unknown session", async () => {
      await expect(
        run("does-not-exist", "oi"),
      ).rejects.toThrow(/sess[aã]o|session|encontrada|expirada/i);
    });

    it("rejects empty user responses via schema", () => {
      const { converseInputSchema } =
        require("../converse.ts") as typeof import("../converse.ts");
      expect(converseInputSchema.safeParse({ session_id: "x", user_response: " " }).success).toBe(false);
      expect(
        converseInputSchema.safeParse({ session_id: "x", user_response: "ola" }).success,
      ).toBe(true);
    });
  });
});
