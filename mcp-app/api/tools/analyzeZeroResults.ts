...
export const analyzeZeroResultsInputSchema = z.object({
	logs: z
		...
		.optional()
		.describe(
			"Logs de buscas com zero resultados (termo + volume). Sem logs, usa demonstração.",
		),
});

export type AnalyzeZeroResultsInput = z.infer<
	typeof analyzeZeroResultsInputSchema
>;
...
export type AnalyzeZeroResultsOutput = z.infer<
	typeof analyzeZeroResultsOutputSchema
>;
...
}> = [
	{
		...
		test: (term, catalogTitles) => {
			...
			return tokens.some((t) => {
				...
				return syns.some((s) =>
					catalogTitles.some((title) =>
						normalize(title).includes(normalize(s)),
					),
				);
			});
		},
		fix: (term) =>
			`Mapear sinônimo: "${term}" → termos do catálogo (ex.: t-shirt, mug, bottle)`,
	},
	{
		cause: "regionalismo",
		test: (term) => {
		...
		fix: (term) =>
			`Adicionar regionalismo: "${term}" → sinônimo local (ex.: chinelo → flip flops)`,
	},
	{
		cause: "typo",
		test: (term) => {
		...
		execute: async ({ context }) => {
			...
			const classified = terms.map(({ term, volume }) => {
				...
				return {
					...
					suggested_fix:
						rule?.fix(term) ?? `Avaliar inclusão no catálogo: "${term}"`,
				};
			});
			...
			try {
				...
				if (err instanceof LlmError) {
					console.warn(
						`[analyze_zero_results] LLM indisponível, regras locais: ${err.message}`,
					);
				}
			}
			...
			const summary =
				...
				top
					.map(
						([c, v]) =>
							`${causeLabel[c] ?? c} (${Math.round((v / total) * 100)}%)`,
					)
					.join(", ") +
