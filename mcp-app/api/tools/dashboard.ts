	...
	createTool({
		...
		execute: async () => {
			...
			const exposedSessions = new Set(
				events
					.filter((e) => e.event === "recova_exposed")
					.map((e) => e.session_id),
			);
			...
			const revenuePerFailedSearch =
				zeroResults > 0
					? Math.round((attributedRevenue / zeroResults) * 100) / 100
					: 0;
			...
			const productsPerUser =
				exposedCount > 0
					? Math.round((productViews / exposedCount) * 10) / 10
					: 0;
			...
			const checkoutRate =
				exposedCount > 0
					? Math.round((checkouts / exposedCount) * 1000) / 10
					: 0;
