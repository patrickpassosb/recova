...
import { z } from "zod";
import { type RecovaEventName, trackEvent } from "../lib/events.ts";
...
export const trackEventInputSchema = z.object({
	...
	trigger: z
		.enum(["zero_results", "low_relevance"])
		.optional()
		.describe("Gatilho da exposição"),
	...
	products_shown: z
		...
		.int()
		.optional()
		.describe("Quantos produtos foram mostrados"),
	product_id: z.string().optional().describe("ID do produto (clique/compra)"),
	price: z
		.number()
		.optional()
		.describe("Preço do produto (para receita atribuída)"),
	exposed_session_id: z
		.string()
		.optional()
		.describe("Sessão exposta que gerou a compra (atribuição)"),
});
