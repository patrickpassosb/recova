...
export default function ConversePage() {
	const state = useMcpState<ConverseInput, ConverseOutput>();
	const app = useMcpApp();
		...
		return (
			<div className="flex items-center justify-center min-h-dvh p-6">
				<div className="flex items-center gap-3 text-muted-foreground">
					...
					<span className="text-sm">Conectando ao loop de conversa...</span>
				</div>
			</div>
		);
	}
			...
			<div className="flex items-center justify-center min-h-dvh p-6">
				<div className="flex items-center gap-3 text-muted-foreground">
					...
					<span className="text-sm">
						Refinando a busca para "{state.toolInput?.user_response ?? ""}"...
					</span>
				</div>
			</div>
		);
				...
				<Card>
					...
					<CardContent className="space-y-4">
						...
						<div className="rounded-lg bg-muted p-3">
							<p className="text-sm">
								<span className="font-medium">Nova pergunta: </span>
							...
							onClick={() => {
								app?.sendMessage({
									...
									content: [
										{
											type: "text",
											text: `O cliente respondeu: "${result.follow_up_question}" — continue com a tool converse (sessão ${result.session_id}).`,
										},
									],
								});
							}}
