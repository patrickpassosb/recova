...
|---|---|---|---|
| MCP agent app | `mcp-app/` | Bun server with tools `search_recovery`, `converse`, `reengage`, `analyze_zero_results`, `track_event`, `dashboard` (DeepSeek V4 Flash via Ollama Cloud) | **Superseded** — V2 runtime is Node+TS ADK on Gemini 3.7 Flash; logic ported as reference, runtime replaced |
...
| V1 docs | `README.md`, `BRIEF.md`, `GOAL.md`, `docs/PRD.md`, `docs/FLUXOGRAMA.md` | Deco-hackathon specs (Portuguese) | Superseded by English V2 docs; kept for provenance |

## Built during the All Things Agentic submission period (V2)

New after `deco-v1-final` (non-exhaustive, auditable via `git log deco-v1-final..HEAD`):

- `agent-service/` — Node + TypeScript Google ADK agent service (Gemini 3.7 Flash)
- Deterministic recovery domain: constraints, routes (`NATIVE_OK/RECOVER/CLARIFY`), strategies, Decision Cards
- ~10k-product stress-test catalog (Apache-2.0 Shopify product-catalogue dataset) + deterministic importer + recovery benchmark
- Merchant console, background search-repair agent, attribution pipeline, CI/CD contract, Cloud Run deployment
- All V2 documentation in English under `docs/`
